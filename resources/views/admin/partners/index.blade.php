...
@section('header_title', 'Partners / Client Logos')
...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Semua Partner</h3>
            <a href="{{ route('admin.partners.create') }}" style="background: var(--accent-orange); color: white; padding: 8px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; text-decoration: none;">+ Tambah</a>
        </div>
            ...
            <table class="table">
                ...
                <tbody>
                    @forelse($partners as $partner)
                        <tr>
                            <td>
                                <img src="{{ str_starts_with($partner->logo, 'http') || str_starts_with($partner->logo, 'assets') ? asset($partner->logo) : Storage::url($partner->logo) }}" alt="Logo" style="height: 40px; object-fit: contain;">
                            </td>
                            <td style="font-weight: 600;">{{ $partner->name }}</td>
                            <td>{{ $partner->order_position }}</td>
                            <td>
                                <span class="badge {{ $partner->is_active ? 'badge-read' : 'badge-unread' }}">
                                    {{ $partner->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td>
                                <div style="display: flex; gap: 8px;">
                                    <a href="{{ route('admin.partners.edit', $partner) }}" class="btn-secondary" style="padding: 6px 14px; font-size: 12px; text-decoration: none;">Edit</a>
