...
<section class="inline-contact-section" style="padding: 80px 40px 120px; border-bottom: none;">
  <div class="ic-container">
    ...
    <div class="ic-info-col">
      ...
      <ul class="ic-contact-list">
        <li>
          ...
          <span>{{ $settings['contact_phone'] ?? '+62 878 8023 1853' }}</span>
        </li>
        <li>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
          <span>{{ $settings['contact_email'] ?? 'info@ksp-consulting.com' }}</span>
        </li>
        <li>
    ...
    <div class="ic-form-col">
      @if(session('success'))
        <div style="background: rgba(13,148,136,0.1); border: 1px solid rgba(13,148,136,0.3); color: #2dd4bf; padding: 14px 18px; border-radius: 12px; margin-bottom: 20px; font-size: 14px;">
          ✓ {{ session('success') }}
        </div>
      @endif
      <form class="ic-form" action="{{ route('contact.submit') }}" method="POST">
        @csrf
        <div class="ic-form-row">
          <input type="text" name="name" class="ic-input" placeholder="Full Name" value="{{ old('name') }}" required>
          <input type="text" name="company" class="ic-input" placeholder="Company" value="{{ old('company') }}">
        </div>
        <div class="ic-form-row">
          <input type="email" name="email" class="ic-input" placeholder="Email" value="{{ old('email') }}" required>
          <input type="tel" name="phone" class="ic-input" placeholder="Phone" value="{{ old('phone') }}">
        </div>
        <textarea name="message" class="ic-textarea" placeholder="Your Message" required>{{ old('message') }}</textarea>
